class AdaptersController < ApplicationController
  before_action :set_adapter, only: [:edit, :update, :destroy]
  ...
  def create
    ...
    respond_to do |format|
      if @adapter.save
        format.html { redirect_to adapters_path, notice: 'Adapter was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @adapter.update(adapter_params)
        format.html { redirect_to adapters_path, notice: 'Adapter was successfully updated.' }
  ...
  private
    ...
    def adapter_params
      params.require(:adapter).permit(:to_plug)
    end
end
